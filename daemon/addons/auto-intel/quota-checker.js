 ...
 *
 * 若快取過期（>2 小時），回退到 ccusage CLI 以美元花費判斷。
 ...
 *
 * @returns {Promise<{five_hour_pct: number|null, seven_day_pct: number|null, age_ms: number}|null>}
 *   回傳快取資料，若檔案不存在、格式錯誤、或已過期則回傳 null。
 */
...
export async function checkQuota({
  ...
  if (cache) {
    const { five_hour_pct, seven_day_pct, age_ms } = cache;
    const ageMin = Math.round(age_ms / 60_000);

    if (five_hour_pct !== null && five_hour_pct >= max5hPct) {
      return {
        ok: false,
